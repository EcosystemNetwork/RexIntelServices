...
const loadFellowship = cache(async (publicId: string) => {
  const [row] = await db
    .select({
      id: submissions.id,
  ...
  return { ...row, payload: row.payload as FellowshipPayload };
});
...
export async function generateMetadata({
  ...
  if (!realId) return { title: "Fellowship not found — Rex Intel Services" };
  const row = await loadFellowship(realId);
  if (!row) return { title: "Fellowship not found — Rex Intel Services" };
  const p = row.payload;
...
export default async function FellowshipDetailPage({
  ...
  if (params.publicId !== canonical) {
    redirect(detailHref("/fellowships", realId, p.name));
  }
  ...
  const jsonLd: Record<string, unknown> = {
    ...
    url: absoluteUrl(detailHref("/fellowships", realId, p.name)),
    provider: {
      "@type": "Organization",
      name: p.organization,
    ...
    educationalProgramMode: p.location?.toLowerCase().includes("remote")
      ? "online"
      : "onsite",
  };

  return (
    <PublicShell classification={[{ text: "● Open Channel // Fellowship Detail" }]}>
      <JsonLd data={jsonLd} />
      <main className="max-w-3xl mx-auto px-6 pt-8 md:pt-12 pb-24">
        <Link
          href="/intel?lane=fellowships"
        ...
        <article className="rex-card p-8">
          ...
          {p.eligibility && (
            <div
              className="mb-6 rounded-sm border p-3 text-sm"
              style={{
