...
The stolen assets were not only Bitcoin. The participant has confirmed that the same drain event lifted **Bitmap Ordinals inscriptions and Runes balances** out of his Xverse wallet alongside the BTC — the kind of multi-asset sweep that an address-book-grade Bitcoin wallet drainer is configured to perform when key material is exfiltrated. The specific destination of those inscriptions has not been confirmed at the time of this update. Ordinals tracing is materially harder than BTC tracing — inscriptions move with the individual satoshi that carries them and require inscription-aware indexing — and most public Ordinals indexers are now either deprecated, gated behind paid API keys, or rate-limited against the kind of address-level enumeration that would be needed to follow specific Bitmaps to their current holders. RexIntel's working assumption is that the inscriptions and Runes either moved through `bc1q704czv9hyce6fg2m9s6c097hg3hue2tzpqwngz` or through a parallel drainer-controlled wallet downstream of the same compromise event; both possibilities remain open.

For reference, the BTC-destination wallet itself is enumerable on **Luminex** ([luminex.io/address/bc1q704czv9hyce6fg2m9s6c097hg3hue2tzpqwngz](https://luminex.io/address/bc1q704czv9hyce6fg2m9s6c097hg3hue2tzpqwngz)), the leading Bitcoin Ordinals marketplace, as an active Ordinals-trading wallet. At the time of this update, it still holds **sixteen distinct Runes balances** alongside a residue of dust sats. RexIntel cannot, at this stage, assert that those particular Runes are the participant's; they may be the operator's accumulated stash from this victim or others. We flag the holding as a forensic lead, not a conclusion.

Bitmap Ordinals — text inscriptions of the form `{block_number}.bitmap`, each representing a land claim in the Bitmap metaverse protocol — carry per-inscription provenance that follows the underlying satoshi across every subsequent transaction. They are, in other words, **the most easily traced category of stolen Bitcoin-native asset that exists** *once the operator-controlled origin wallet is known*. Until that wallet is identified for these specific inscriptions, the trail is open. We invite Ordinals-forensics counterparties (see CTA below) to assist.
...
**If you have Bitcoin Ordinals tracing capability** (Hiro, UniSat, Luminex, Magic Eden, OKX, Ordiscan, ord-running indexers, or independent Ordinals forensics analysts) — the participant has confirmed that Bitmap inscriptions and Runes balances were drained in the same Feb 20-21 2025 event documented in this article. The destination wallet for the inscriptions specifically has not been confirmed. We are seeking partnership on the inscription-side trace and will provide the participant's wallet address, the relevant Bitmap block numbers (where known), and the timing window on a credentialed basis. Email **rexintelservices@proton.me**.

**If you have bought a Bitmap inscription whose provenance traces to a Feb 20-21 2025 transfer out of a Xverse-class wallet** — particularly if the inscription's transfer history includes a hop through any of the addresses named in this article — you are not the target of this report, but the asset's provenance may be relevant. Email **rexintelservices@proton.me** and we will cross-reference against the participant's loss record.
