/**
 * Refresh import for NYC Tech Week 2025 — Innovate for People + Planet: Fintech & AI.
 *
 * Source: /tmp/nytw-innovate-refresh.csv (post-event re-export, June 7).
 ...
 *   - source = event-nytw-2025-innovate-fintech-ai (preserved from prior import)
 *   - tags   = umbrella `nyc-tech-week-2025` + per-event `nytw-2025-innovate-fintech-ai`
 *
 * Run:  npx tsx --env-file=.env scripts/import-nytw-innovate-refresh.ts
 */
...
const CSV_PATH = "/tmp/nytw-innovate-refresh.csv";
const SOURCE_SLUG = "event-nytw-2025-innovate-fintech-ai";

const EVENT_TAG = {
  name: "nytw-2025-innovate-fintech-ai",
  description:
...
async function main() {
  console.log("Refreshing NYTW 2025 Innovate Fintech/AI guest list…\n");
  ...
  const toInsert = Array.from(byEmail.values()).map((v) => ({
    ...
    metadata: {
      ...
      importedFrom: "nytw-2025-innovate-fintech-ai-refresh",
